...
public:
   ...
   //   heart_data_index_(-1)
  {
   ...
   // memset(heart_data_, 0, 64);
    bip_buffer_.AllocateBuffer(write_buffer_size);
  }
...
public:
    ...
    //连接的角色  1:教师  2:学生
    unsigned short user_type_;
    //用户ID
    unsigned int user_id_;
    //用户上的课程id
    unsigned int class_id_;
    ////心跳数据
    //unsigned char heart_data_[64];      
    ////数据长度  
    //unsigned short heart_data_len_;
    //接收循环缓冲
    BipBuffer bip_buffer_;

    //上次io时间
    unsigned int last_io_time_;
    //心跳数据Index
    //int heart_data_index_;
